...
import 'update_employee_form_state.dart';

class UpdateEmployeeFormCubit extends Cubit<UpdateEmployeeFormState> {
  UpdateEmployeeFormCubit({required EmployeeEntity employee})
    : super(
        UpdateEmployeeFormState(
          employeeId: employee.id ?? 0,
          // توحيد وتنظيف المعرفات بشكل صارم عند الدخول لمنع أي تضارب أنواع لاحقاً
          ...
          birthdate: _parseAndFormatDate(employee.birthdate),
          joinedAt: _parseAndFormatDate(employee.joinedAt),
  ...
  static String _parseAndFormatDate(String dateStr) {
  ...
  void updateConfirmPassword(String value) => emit(
  ...
  void clearRoleError() => emit(state.copyWith(clearRoleError: true));
  void clearProfileImageError() =>
      emit(state.copyWith(clearProfileImageError: true));
  void clearNameError() => emit(state.copyWith(clearNameError: true));
  ...
  void clearBirthdateError() => emit(state.copyWith(clearBirthdateError: true));
  void clearJoinedAtError() => emit(state.copyWith(clearJoinedAtError: true));
  void clearPhoneError() => emit(state.copyWith(clearPhoneError: true));
  ...
  void toggleDepartment(int departmentId) {
    ...
    emit(state.copyWith(departmentIds: updated, clearDepartmentError: true));
  }
  ...
  bool validate({required bool isArabic}) {
    ...
    // 💡 التعديل هنا: نعتبر الصورة صالحة إذا كان هناك بايتس جديدة OR مسار قديم غير فارغ
    final dynamicHasImage =
        state.profileImageBytes != null ||
        (state.profileImagePath != null &&
            state.profileImagePath!.trim().isNotEmpty);

    final profileImageError = dynamicHasImage
        ? null
    ...
    // التعديل الذكي هنا: نتحقق من كلمة المرور فقط إذا قام المستخدم بكتابة شيء ولم تكن فارغة أو نقاط وهمية
    final cleanPassword = state.password.trim();
    if (cleanPassword.isNotEmpty && cleanPassword != '••••••••') {
      passwordError = AppValidators.validatePassword(
        state.password,
