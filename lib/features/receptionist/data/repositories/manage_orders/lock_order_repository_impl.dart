...
@Injectable(as: LockOrderRepository)
class LockOrderRepositoryImpl
    implements LockOrderRepository {
  const LockOrderRepositoryImpl(
  ...
  Future<Either<AppFailure, BaseResponseModel>> call({
    ...
    try {
      final response = await remoteDataSource.lockOrder(orderId);

      return Right(response);
    } catch (error, stackTrace) {
      try {
        log(
          'LockOrderRepositoryImpl caught error: ${error.runtimeType}',
        );
        log(error.toString());
